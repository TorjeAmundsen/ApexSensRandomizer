...
def check_for_update(release):
    ...
    if response.status_code == 200:
        if tag != release:
            ...
            torjeLabel.grid(column=1, sticky=tk.E, padx=9)
            updateLabel = tk.Label(window, text=f"New update available! {tag}", fg="#910483", cursor="hand2")
            updateLabel.grid(row=8, column=0, columnspan=2, padx=3, pady=2, sticky=tk.W)
            updateLabel.bind("<Button-1>", lambda e: openSite(f"https://github.com/{owner}/{repo}/releases/latest"))
            f = font.Font(updateLabel, updateLabel.cget("font"))
            f.configure(underline=True)
            updateLabel.configure(font=f)
...
def toggle():
    ...
    startStopLog = ["Randomizer stopped!\n", "Randomizer started!"]
    ...
    disabledElementsWhileRunning = [gameDir, selectFolder, dpiEntry, minSensEntry,
                                    ...
                                    useTimerBox, timerIntervalEntry, autoDetectDir,
    ...
    sensLog.write(f"\n[{now.strftime('%Y-%m-%d %H:%M:%S] ')}{startStopLog[running]}")
...
def reset_sensitivity():
    randomsens = open(directory.get() + "/cfg/randomsens.cfg", "w")
    randomsens.write(f"mouse_sensitivity {baseSens.get()}")
...
def generateAutoExec():
    saveConfig()
    enablerStr = f"""#Automatically generated by Apex Sens Randomizer
...
bind \"{disableBind.get()}\" \"exec disablerando; exec autoexec\""""
    ...
    try:
        with open(directory.get() + "/cfg/autoexec.cfg", 'r+') as autoexec:
            ...
            if line_number is not None:
                lines[line_number] = f"bind \"{enableBind.get()}\" \"exec enablerando\"\n"
            with open(directory.get() + "/cfg/autoexec.cfg", 'w') as autoexec:
                autoexec.writelines(lines)
    except FileNotFoundError:
        with open(rf"{directory.get()}/cfg/autoexec.cfg", 'x') as autoexec:
...
def loadConfig():
#    darktheme.theme_create("darkmode", parent="vista", settings={
#        "TCombobox": {
#            "configure": {
#                "background": baseBackgroundColors[1],
#                "fieldbackground": interactableBackgroundColors[1],
#                "foreground": "white"
#            }
#        }
#    })
...
""" def toggleTheme():
    ...
    darktheme.theme_use(themeList[theme.get()])
...
window = tk.Tk()
style = ttk.Style()
window.title("Apex Sens Randomizer v1.0.1")
...
twitchLink = tk.Label(window, text="Twitch", fg="#572da6", cursor="hand2")
twitchLink.grid(row=8, column=2, columnspan=2, pady=2, sticky=tk.W, padx=70)
...
youtubeLink.grid(row=8, column=3, pady=2, padx=4, sticky=tk.E)
...
twitterLink.grid(row=8, column=2, columnspan=2, pady=2, sticky=tk.W)
twitterLink.bind("<Button-1>", lambda e: openSite("https://twitter.com/Txrje"))


window.bind("<Visibility>", lambda e: check_for_update(releaseTag))


""" themeButton = tk.Button(window, text="Theme", command=toggleTheme)
themeButton.grid(row=8, column=0, padx=24, pady=4, sticky=tk.W)

updateTemp = tk.Button(window, text="Update", command=lambda: check_for_update(releaseID))
updateTemp.grid(row=0, column=0) """
